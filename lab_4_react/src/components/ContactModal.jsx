  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-slate-900 rounded-3xl p-8 max-w-md w-full shadow-2xl relative overflow-hidden">
        ...
        <h2 className="text-2xl font-bold mb-4 text-slate-900 dark:text-white">Зворотній зв'язок</h2>
        <p className="text-sm text-slate-600 dark:text-slate-400 mb-6">Ви на сайті вже хвилину! Маєте питання до розробника?</p>
        
        {}
        <form action="https://formspree.io/f/mlgoaojz" method="POST" className="space-y-4">
          <input 
            type="text" 
            name="name" 
            placeholder="Ім'я" 
            required 
            className="w-full p-3 rounded-xl border text-slate-900 dark:text-white dark:bg-slate-800 dark:border-slate-700 outline-none focus:ring-2 ring-emerald-500" 
          />
          <input 
            type="email" 
            name="email" 
          ...
          <textarea 
            ...
            required
            className="w-full p-3 rounded-xl border text-slate-900 dark:text-white dark:bg-slate-800 dark:border-slate-700 h-24 outline-none focus:ring-2 ring-emerald-500"
          ></textarea>
